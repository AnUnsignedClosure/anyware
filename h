...
--[=[
	Constructs a new Spring at the position and target specified.
	Works with numbers, Vector2, Vector3, etc.
]=]
	function Spring.new(initial, clock)
		...
		local self = setmetatable({
			...
			_time0 = springClock(),
			_position0 = p0,
			_velocity0 = p0 * 0, -- Ensures velocity starts as the same type (Vector3, etc)
			_target = p0,
			_damper = 1,
			_speed = 1,
		}, Spring)
	...
	function Spring:Impulse(velocity)
		...
		local pos, vel = self:_positionVelocity(now)

		self._position0 = pos
		self._velocity0 = vel + velocity
		self._time0 = now
	end
	...
	function Spring:_positionVelocity(now)
		...
		if t <= 0 then return p0, v0 end

		local d2 = d * d
		local h, si, co

		if d2 < 1 then -- Underdamped (Bouncy)
			h = math.sqrt(1 - d2)
			local ep = math.exp(-d * t) / h
			co, si = ep * math.cos(h * t), ep * math.sin(h * t)
		elseif d2 == 1 then -- Critically Damped
	...
	Spring.__index = function(self, index)
		-- Check if the index is a method (Impulse, SetTarget, etc)
		if Spring[index] then return Spring[index] end

		local now = self._clock()
		if index == "Position" or index == "Value" or index == "p" then
			local pos, _ = self:_positionVelocity(now)
			return pos
		elseif index == "Velocity" or index == "v" then
			local _, vel = self:_positionVelocity(now)
			return vel
	...
	Spring.__newindex = function(self, index, value)
		local now = self._clock()
		-- Sync physical state before changing parameters
		local pos, vel = self:_positionVelocity(now)
		self._position0 = pos
		self._velocity0 = vel
		self._time0 = now
